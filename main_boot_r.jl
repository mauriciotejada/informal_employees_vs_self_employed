...
function main(sk_lev)
    ...
    Threads.@threads for i in 1:Nboot
        ...
        res_est = optimize(Θ ->  likefun_r(Θ, db_sk_lev_boot), x0, 
                                    NelderMead(),
                                    Optim.Options(show_trace = false,  
                                                        g_tol = 1e-3,
                                                        iterations = 15000))        

        param_boot[:,i] = trans_r(res_est.minimizer)

    end

    stderror = std(param_boot,dims=2)
    writedlm("res/stderror_r_boot_"*string(Int(sk_lev))*".txt", stderror)
    writedlm("res/est_param_r_boot_"*string(Int(sk_lev))*".txt", param_boot)
